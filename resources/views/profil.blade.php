...
<div class="container mx-auto p-4">
    ...
    <div class="flex flex-col gap-4">
        ...
        <form onsubmit="submitEditForm(event)" method="POST" action="{{ route('pasien.profil') }}" class="flex justify-center md:text-lg">
            ...
            <div class="w-full md:max-w-4xl flex flex-col items-center gap-5">
                ...
                <div class="w-full flex flex-col md:flex-row gap-4">
                    ...
                    <div class="flex flex-col w-full">
                        <label class="font-semibold mb-1" for="alamat">Jenis Kelamin</label>
        ...
        <p class="md:text-md">Ubah kata sandi untuk mengamankan akun anda</p>
        ...
        <p class="text-justify md:text-md">Setelah akun Anda dihapus, maka anda tidak dapat lagi mengakses informasi dan fitur dalam website klinik RH61 ini, silahkan daftarkan diri anda lagi jika ingin melihat kembali informasi dan mengakses kembali website ini.</p>
        <form onsubmit="hapusAkun(event)" method="POST" action="{{ route('akun.destroy') }}">
            @csrf
            @method('DELETE')
            <button type="submit" class="border-2 rounded-lg bg-gray-200 border-gray-300 text-[#B42223] hover:text-white hover:bg-[#B42223] hover:border-[#B42223] font-semibold px-6 py-2 w-min text-nowrap"><i class="fa-solid fa-trash mr-2"></i>Hapus akun anda</button>
        </form>
    </div>
</div>
@push('scripts')
