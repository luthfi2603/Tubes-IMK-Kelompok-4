...
@if(session()->has('success'))
    <div id="success-php" class="bg-[#d1e7dd] dark:bg-green-900 text-[#0f5132] dark:text-green-300 border-2 border-[#badbcc] dark:border-green-700 px-4 py-3 rounded-lg fixed inset-x-[296px] z-[999]">
        <i class="fa-regular fa-circle-check mr-1"></i>
        <span>{{ session('success') }}</span>
    </div>
@elseif(session()->has('failed'))
    <div id="failed-php" class="bg-[#f8d7da] dark:bg-red-900 text-[#842029] dark:text-red-300 border-2 border-[#f5c2c7] dark:border-red-700 px-4 py-3 rounded-lg fixed inset-x-[296px] z-[999]">
                            ...
                            @if($reservasi->status == 'Menunggu')
                                <div class="flex gap-2 mt-2">
                                    <form method="POST" action="{{ route('destroy.reservasi') }}">
                                        ...
                                        <button class="bg-[#b02126] hover:bg-red-500 dark:bg-red-900 dark:hover:bg-red-600 rounded-lg py-1.5 px-3 text-white w-min">Batalkan</button>
                                    </form>
                                    <a href="{{ route('reservasi.edit', $reservasi->id) }}" class="bg-yellow-500 hover:bg-yellow-400 dark:bg-yellow-900 dark:hover:bg-yellow-600 rounded-lg py-1.5 px-3 text-white w-min text-nowrap">Ubah</a>
                                </div>
                            @elseif($reservasi->status == 'Selesai')
                                @if($reservasi->id_rekam_medis)
                                    <a href="{{ route('rekam.medis.detail', $reservasi->id_rekam_medis) }}" class="bg-blue-500 hover:bg-blue-400 dark:bg-blue-900 dark:hover:bg-blue-600 rounded-lg py-1 px-3 text-white w-min mt-2 text-nowrap">Rekam Medis</a>
                                @else
